...
public class RedisKeyExpiredListener extends KeyExpirationEventMessageListener {
	...
	public RedisKeyExpiredListener(RedisMessageListenerContainer listenerContainer, QueueManagementFacade queueManagementFacade) {
		super(listenerContainer);
		this.queueManagementFacade = queueManagementFacade;
		startBatchProcessor();
	}
	...
	private void processExpiredKeys() {
		try {
			while (expiredKeysQueue.size() >= BATCH_SIZE) {
				...
				queueManagementFacade.completeTokens(expireCommand(keys));
			}
		} catch (InterruptedException e) {
			Thread.currentThread().interrupt();
