...
public class ReservationStatusSimulation extends Simulation {
	...
	private ScenarioBuilder createScenario() {
		return scenario("Reservation Status Scenario")
			.exec(session -> {
				Map<String, Object> params = generateParams();
				return session.setAll(params);
			})
			.exec(http("예약 상태 조회")
				.get("/api/reservations/status/#{userId}/#{concertOptionId}")
				.check(status().is(200))
			)
			.pause(2);
	}
	...
	private Map<String, Object> generateParams() {
		...
		int randomId = ThreadLocalRandom.current().nextInt(1, 100); // 1부터 1,000,000 사이의 랜덤 ID 생성
		params.put("userId", randomId);
		params.put("concertOptionId", randomId); // userId와 concertOptionId가 동일한 값으로 설정 <- 더미데이터와 align
		return params;
	}
}
